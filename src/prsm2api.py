...
# --------- Start of getStoragePortProperties function ------------- #
# ------------------------------------------------------------------ #
def getStoragePortProperties(hmcConn,
                             adaPortURI=None):
    log.debug("Entered")
    try:
        # check input params
        if adaPortURI != None:
            ...
            exc = HMCException("getStoragePortProperties",
                               "You should specify the adaPortURI parameters")
            raise exc
        # Get Storage Port Properties
        return getHMCObject(hmcConn,
                            URI,
        ...
        exc.setMethod("getStoragePortProperties")
        raise exc
    finally:
        log.debug("Completed")
# ------------------------------------------------------------------ #
# ----------- End of getStoragePortProperties function ------------- #
# ------------------------------------------------------------------ #

# --------- Start of getStorageSwitchProperties function ----------- #
# ------------------------------------------------------------------ #
def getStorageSwitchProperties(hmcConn,
                               storageSwitchURI=None):
    log.debug("Entered")
    try:
        # check input params
        if storageSwitchURI != None:
            URI = storageSwitchURI
        else:
            exc = HMCException("getStorageSwitchProperties",
                               "You should specify the storageSwitchURI parameters")
            raise exc
        ...
        exc.setMethod("getStorageSwitchProperties")
        raise exc
    finally:
        log.debug("Completed")
# ------------------------------------------------------------------ #
# ----------- End of getStorageSwitchProperties function ----------- #
# ------------------------------------------------------------------ #
...
# ------------------------------------------------------------------ #
def createTapeLinks(hmc, tlTempl):
    try:
        # prepare HTTP body as JSON
...
# ------------------------------------------------------------------ #
def listPartitionLinks(hmcConn, query=None):
...
# ------------------------------------------------------------------ #
def getPartitionLinkProperties(hmcConn, plID=None, plURI=None):
    try:
        # check input params
        if plID != None:
...
# ------------------------------------------------------------------ #
# ----- Start of createPartitionLinks function ----------------- #
# ------------------------------------------------------------------ #
def createPartitionLinks(hmc, plTempl):
    try:
        # prepare HTTP body as JSON
        ...
        print ("[HMCEXCEPTION createPartitionLinks]", exc.message)
        if exc.httpResponse != None:
            print ("[HMCEXCEPTION createPartitionLinks]", eval(exc.httpResponse)['message'])
        raise exc
    except Exception as exc:
        print ("[EXCEPTION createPartitionLinks]", exc)
        raise exc

# ------------------------------------------------------------------ #
# ----- End of createPartitionLinks function ------------------- #
# ------------------------------------------------------------------ #

# ------------------------------------------------------------------ #
# --------------- Start of modifyPartitionLinks function ----------- #
# ------------------------------------------------------------------ #
def modifyPartitionLinks(hmc, plID, plTempl):
    try:
        # prepare HTTP body as JSON
        httpBody = json.dumps(plTempl)
        # create workload
        resp = getHMCObject(hmc, 
                            WSA_URI_MODIFY_PARTITION_LINK % plID, 
                            "Modify Partition Link", 
                            httpMethod = WSA_COMMAND_POST, 
                            httpBody = httpBody, 
                            httpGoodStatus = 202,           # HTTP created
                            httpBadStatuses = [500, 503])
        # return assertValue(pyObj=resp, key='object-uri')
        return resp
    except HMCException as exc:   # raise HMCException
        print ("[HMCEXCEPTION modifyPartitionLinks]", exc.message)
        if exc.httpResponse != None:
            print ("[HMCEXCEPTION modifyPartitionLinks]", eval(exc.httpResponse)['message'])
        raise exc
    except Exception as exc:
        print ("[EXCEPTION modifyPartitionLinks]", exc)
        raise exc
# ------------------------------------------------------------------ #
# ------- End of modifyPartitionLinks function --------------------- #
# ------------------------------------------------------------------ #

# ------------------------------------------------------------------ #
# --------------- Start of deletePartitionLinks function ------- #
# ------------------------------------------------------------------ #
def deletePartitionLinks(hmcConn, plID=None, plTempl=None):
    try:
        # check input params
        if plID == None:
            exc = HMCException("deletePartitionLink",
                               "you should specify partition link ID parameter")
            raise exc
        else:
            # prepare HTTP body as JSON
            httpBody = json.dumps(plTempl)
            # delete the partition link
            '''
            Once the delete request is accepted,
            a 202 (Accepted) status code is returned and the response body contains a URI that may be queried to
            retrieve the status of the operation
            '''
            resp = getHMCObject(hmcConn,
                                WSA_URI_DELETE_PARTITION_LINK % plID,
                                "Delete partition link",
                                httpMethod=WSA_COMMAND_POST,
                                httpBody=httpBody,
                                httpGoodStatus=202,           # HTTP created
                                httpBadStatuses=[403, 404, 409, 503])
            return assertValue(pyObj=resp, key='job-uri')
    except HMCException as exc:   # raise HMCException
        exc.setMethod("deletePartitionLinks")
        raise exc
    finally:
        log.debug("deletePartitionLinks")
# ------------------------------------------------------------------ #
# ----------------- End of deletePartitionLinks function ------- #
# ------------------------------------------------------------------ #

# ---------------------------------------------------------------------------------------- #
# ----------------- For Secure Boot Certificate Management ------------------------------- #
# ---------------------------------------------------------------------------------------- #

# ------------------------------------------------------------------ #
# ------- Start of importCpcCertificates function ------------------ #
# ------------------------------------------------------------------ #
def importCpcCertificates(hmcConn, cpcID, certTempl):
    try:
        # prepare HTTP body as JSON
        httpBody = json.dumps(certTempl)

        resp = getHMCObject(hmcConn, 
                            WSA_URI_IMPORT_CPC_CERTIFICATE % cpcID,
                            "Import CPC Certificate", 
                            httpMethod = WSA_COMMAND_POST, 
                            httpBody = httpBody, 
                            httpGoodStatus = 200,
                            httpBadStatuses = [400, 403, 404, 409, 503])
        return assertValue(pyObj=resp, key='certificate-uri')
    except HMCException as exc:   # raise HMCException
        print ("[HMCEXCEPTION importCpcCertificates]", exc.message)
        if exc.httpResponse != None:
            print ("[HMCEXCEPTION importCpcCertificates]", eval(exc.httpResponse)['message'])
        raise exc
    except Exception as exc:
        print ("[EXCEPTION importCpcCertificates]", exc)
        raise exc

# ------------------------------------------------------------------ #
# ------- End of importCpcCertificates function -------------------- #
# ------------------------------------------------------------------ #

# ------------------------------------------------------------------ #
# ------- Start of getCertificateProperties function --------------- #
# ------------------------------------------------------------------ #
def getCertificateProperties(hmcConn, certID = None):
    try:
        resp = getHMCObject(hmcConn, 
                            WSA_URI_GET_UPDATE_DELETE_CPC_CERTIFICATE % certID,
        ...
        print ("[HMCEXCEPTION getCertificateProperties]", exc.message)
        if exc.httpResponse != None:
            print ("[HMCEXCEPTION getCertificateProperties]", eval(exc.httpResponse)['message'])
        raise exc
    except Exception as exc:
        print ("[EXCEPTION getCertificateProperties]", exc)
        raise exc

# ------------------------------------------------------------------ #
# ------- End of getCertificateProperties function ----------------- #
# ------------------------------------------------------------------ #

# ------------------------------------------------------------------ #
# ------- Start of updateCpcCertificates function ------------------ #
# ------------------------------------------------------------------ #
def updateCpcCertificates(hmcConn, certID = None, certTempl = None):
    # prepare HTTP body as JSON
    httpBody = json.dumps(certTempl)
    ...
    try:
        ...
        print ("[HMCEXCEPTION updateCpcCertificates]", exc.message)
        if exc.httpResponse != None:
            print ("[HMCEXCEPTION updateCpcCertificates]", eval(exc.httpResponse)['message'])
        raise exc
    except Exception as exc:
        print ("[EXCEPTION updateCpcCertificates]", exc)
        raise exc

# ------------------------------------------------------------------ #
# ------- End of updateCpcCertificates function -------------------- #
# ------------------------------------------------------------------ #

# ------------------------------------------------------------------ #
# ------- Start of deleteCpcCertificates function ------------------ #
# ------------------------------------------------------------------ #
def deleteCpcCertificates(hmcConn, certID = None):
    try:
        resp = getHMCObject(hmcConn, 
                            WSA_URI_GET_UPDATE_DELETE_CPC_CERTIFICATE % certID,
                            "Delete CPC Certificate", 
                            httpMethod = WSA_COMMAND_DELETE, 
                            httpBody = None, 
                            httpGoodStatus = 204,
                            httpBadStatuses = [403, 404, 409, 503])
        ...
        print ("[HMCEXCEPTION deleteCpcCertificates]", exc.message)
        if exc.httpResponse != None:
            print ("[HMCEXCEPTION deleteCpcCertificates]", eval(exc.httpResponse)['message'])
        raise exc
    except Exception as exc:
        print ("[EXCEPTION deleteCpcCertificates]", exc)
        raise exc

# ------------------------------------------------------------------ #
# ------- End of deleteCpcCertificates function -------------------- #
# ------------------------------------------------------------------ #

# ------------------------------------------------------------------ #
# ------- Start of listCertificates function ----------------------- #
# ------------------------------------------------------------------ #
def listCertificates(hmcConn, query=None):
    uri = WSA_URI_LIST_CERTIFICATE
    if query != None:
        uri += '?' + query
    try:
        # list certificates
        return getHMCObjectList(hmcConn,
                                ...
                                "List certificates",
                                "certificates",
                                httpBadStatuses=[400, 403])
    except HMCException as exc:   # raise HMCException
        exc.setMethod("List certificates")
        raise exc
    finally:
        log.debug("Completed")
...
# ------------------------------------------------------------------ #
# ------- Start of assignCertificateToPartitions function ---------- #
# ------------------------------------------------------------------ #
def assignCertificateToPartitions(hmcConn, partID, certTempl):
    try:
        # prepare HTTP body as JSON
        httpBody = json.dumps(certTempl)

        resp = getHMCObject(hmcConn, 
                            WSA_URI_ASSIGN_CERTIFICATE_TO_PARTITION % partID,
        ...
        print ("[HMCEXCEPTION assignCertificateToPartitions]", exc.message)
        if exc.httpResponse != None:
            print ("[HMCEXCEPTION assignCertificateToPartitions]", eval(exc.httpResponse)['message'])
        raise exc
    except Exception as exc:
        print ("[EXCEPTION assignCertificateToPartitions]", exc)
        raise exc

# ------------------------------------------------------------------ #
# ------- End of assignCertificateToPartitions function ------------ #
# ------------------------------------------------------------------ #

# ------------------------------------------------------------------ #
# ------- Start of unassignCertificateFromPartitions function ------ #
# ------------------------------------------------------------------ #
def unassignCertificateFromPartitions(hmcConn, partID, certTempl):
    try:
        # prepare HTTP body as JSON
        httpBody = json.dumps(certTempl)

        resp = getHMCObject(hmcConn, 
                            WSA_URI_UNASSIGN_CERTIFICATE_FROM_PARTITION % partID,
        ...
        print ("[HMCEXCEPTION unassignCertificateFromPartitions]", exc.message)
        if exc.httpResponse != None:
            print ("[HMCEXCEPTION unassignCertificateFromPartitions]", eval(exc.httpResponse)['message'])
        raise exc
    except Exception as exc:
        print ("[EXCEPTION unassignCertificateFromPartitions]", exc)
        raise exc

# ------------------------------------------------------------------ #
# ------- End of assignCertificateToPartition function ------------- #
# ------------------------------------------------------------------ #